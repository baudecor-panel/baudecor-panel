...
export default function StockOutPage() {
  ...
  async function syncProductStockById(productId: string) {
  ...
  async function handleReverseMovement(movement: StockMovement) {
    ...
    try {
      ...
      await syncProductStockById(movement.product_id);
  ...
  async function fetchMovements() {
    ...
    const { data, error } = await supabase
      ...
      .in("movement_type", [
        ...
        "Showroom Kullanım / Showroom Use",
      ])
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (stockError) {
        throw new Error(
          "Ažuriranje zalihe nije uspjelo / Stok güncellenemedi: " + stockError.message
        );
      }
      ...
      alert(
        error instanceof Error
          ? error.message
          : "Operacija nije uspjela / İşlem başarısız"
      );
    } finally {
      setSaving(false);
    }
                  ...
                  >
                    ...
                    <td className="py-3 text-center">
                      <button
                        ...
                        className="rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 text-xs font-medium text-amber-300 transition hover:bg-amber-500/20 disabled:cursor-not-allowed disabled:opacity-40"
                      >
